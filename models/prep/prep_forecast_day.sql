...
add_features AS (
    SELECT *
        ...
        ,TO_CHAR('month_of_year', VARCHAR(255)) AS month_of_year -- month name as a text
        ,DATE_PART('year', INT) AS year -- year as a number
        ,TO_CHAR('day_of_week', VARCHAR(255)) AS day_of_week -- weekday name as text
        ,DATE_PART('week_of_year', INT) AS week_of_year -- calender week number as number
        ,TO_CHAR('year_and_week', VARCHAR(255)) AS year_and_week -- year-calenderweek as text like '2024-43'
